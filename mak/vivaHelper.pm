...
use strict;
use warnings;
BEGIN { push @INC, '.\\mak\\' }  # add additional library path
use FileInfoParser;
use sysGenUtility;
use LISInfo;
use CommonUtility;
...
# History
#****************************************************************************
my $VIVA_HELPER_VERNO = " v1.03";
   # v1.03: Support region config info processing
   # v1.02: Support RO-CODE/RO-DATA wildcard
   # v1.01: Support merging duplicated entries
   # v1.00: Phase in VIVA
...
sub Initialize
{
   ...
   if (defined $makefilePath)
   {
      ...
      {
         &ErrorHandler("Fail to parse makefile $makefilePath $!") ;
      }
...
sub ReadGFHHeader
{
   ...
   &ErrorHandler ("Error GFH header magic number!")
...
sub ReadGFHFileInfo
{
   ...
   &ErrorHandler("Invalid GFH file info header!")
...
sub UpdateGFHFileInfo
{
   ...
   open my $inout, "+<$binary" or &ErrorHandler("Cannot open binary $binary: $!");
   binmode $inout;
   ...
   {
      &ErrorHandler("Unrecognizable GFH file info entry $key!")
...
sub ReadVIVAInfoFromBinary_NI
{
   ...
   open my $input, "<$binary" or
      &ErrorHandler("Cannot open binary $binary: $!");
   binmode $input;
...
sub SplitBinaryBySize
{
   ...
   #&ErrorHandler("The size number is different to filename number!")
   #   if (scalar(@$sizeArrayRef) != scalar(@$filenameArrayRef));

   open my $input, "<$binary" or &ErrorHandler("Cannot open binary $binary: $!");
   binmode $input;
   ...
   for (my $i = 0; $i < scalar(@$sizeArrayRef); ++$i)
   {
      ...
      {
         &ErrorHandler("Invalid filename while split file!")
      }

      open my $output, ">$filenameArrayRef->[$i]"
         or &ErrorHandler("Cannot open binary $filenameArrayRef->[$i]: $!");
      binmode $output;
...
sub ConcatenateBinary
{
   ...
   {
      open my $input, "<$source" or &ErrorHandler("Cannot open binary $source: $!");
      binmode $input;
...
sub PaddingBinaryToAlignment
{
   ...
   open my $output, ">>$binary" or &ErrorHandler("Cannot open binary $binary: $!");
   binmode $output;
...
#****************************************************************************
# subroutine:  ReadRegionConfigInfoHash
# description: Read the region configuration information
# input:       Region name, custom folder, major index, require index list, info hash ref
# output:      Info hash
# need init:   No
#****************************************************************************
sub ReadRegionConfigInfoHash
{
   my $regionName = shift;
   my $customFolder = shift;
   my $majorIndex = shift;
   my $requireIndexList = shift;
   my $infoRef = shift;

   my $regionConfigFileName = "$regionName.csv";

   my $regionConfigFilePath = &scatFrame::GetConfigInput(undef, $regionConfigFileName, scatFrame::Regions, $customFolder);

   # Read and parse CSV file
   {
      my ($regionConfigContentList, $regionConfigIndex) = &CommonUtil::ParseCSV($regionConfigFilePath, undef);
      my @indexList;

      if (defined $requireIndexList)
      {
         @indexList = @$requireIndexList;
      }
      else
      {
         my %tempHash = %$regionConfigIndex;
         delete $tempHash{$majorIndex};
         @indexList = keys(%tempHash);
      }

      foreach my $row (@$regionConfigContentList)
      {
         my $majorIndexInfo = $row->[$regionConfigIndex->{$majorIndex}];
         foreach my $column (@indexList)
         {
            $infoRef->{$majorIndexInfo}{$column} = $row->[$regionConfigIndex->{$column}];
         }
      }
   }
}
...
#****************************************************************************
# subroutine:  UpdateRegionConfigInfoHash
# description: Update the region configuration information
# input:       Region name, custom folder, major index, info hash ref
# output:      None
# need init:   No
#****************************************************************************
sub UpdateRegionConfigInfoHash
{
   my $regionName = shift;
   my $customFolder = shift;
   my $majorIndex = shift;
   ...
   # Parse and update info
   {
      ...
      {
         my $majorIndexInfo = $row->[$regionConfigIndex->{$majorIndex}];
         if (defined $infoRef->{$majorIndexInfo})
         {
            while (my ($key, $value)= each %{$infoRef->{$majorIndexInfo}})
            {
               $row->[$regionConfigIndex->{$key}] = $value;
            }
         }
      }
      ...
      # Write to CSV file
      {
         my $regionConfigOutputFilePath = &scatFrame::GetCustomInput(undef, $regionConfigFileName, scatFrame::Regions, $customFolder);

         unless ($regionConfigFilePath eq $regionConfigOutputFilePath)
         {
            my($filename, $directory, $suffix) = fileparse($regionConfigOutputFilePath);
            unless (-d $directory)
            {
                mkpath($directory)
                  or &ErrorHandler("Fail to create folder $directory $!");
            }

            copy($regionConfigFilePath, $regionConfigOutputFilePath) or
               &ErrorHandler("Cannot copy file $regionConfigFilePath to $regionConfigOutputFilePath: $!");
         }
         &BackupFile($regionConfigOutputFilePath);

         &CommonUtil::WriteCSV($regionConfigOutputFilePath, $regionConfigContentList, $regionConfigIndex) or
            &ErrorHandler("Cannot update config file $regionConfigOutputFilePath: $!");
      }
   }
}
...
#****************************************************************************
# subroutine:  GetAllLISLoadRegionInfo
# description: Get all the LIS load region information
# input:       Info hash ref
# output:      Info hash
# need init:   Yes
#****************************************************************************
sub GetAllLISLoadRegionInfo
{
   my $infoRef = shift;
   my $result = &LISInfo::GetAllLoadRegionRawInfo();

   foreach my $value (@$result)
   {
      my %info;

      $info{"region_name"} = $value->[0];
      $info{"base_address"} = $value->[1];
      $info{"actual_size"} = $value->[2];
      $info{"max_size"} = $value->[3];
      $info{"attribute"} = $value->[4];
...
# description: Get the maximum allowable size in the current load
# input:       File system base
# output:      Max allowable size
# need init:   Yes
#****************************************************************************
sub GetVIVAMaxAllowSize
{
   my $fsBase = shift;
   return &LISInfo::GetVIVAMaxAllowSize(\%makefileOptions, $fsBase);
}

#****************************************************************************
# subroutine:  GetDcmcmpTotalRAMSize
# description: Get the RAM size used by DCMCMP
# input:       Custom folder
# output:      RAM size used by DCMCMP
# need init:   Yes
#****************************************************************************
sub GetDcmcmpTotalRAMSize
{
   my $customFolder = shift;
   &scatUtil::ParseDCMP($customFolder);
   my %dontcare;
   return &scatUtil::DCMP_GetQueryInfo(0x0, &scatUtil::DCMP_GetCombination() , \%dontcare );
}
...
#****************************************************************************
# subroutine:  MergeDuplicateObjectListByName
# description: Merge the duplicate object list entries by their names
# input:       Source list ref, name list,
#              result list ref, the removed entry list
# output:      The result and merged list
# need init:   No
#****************************************************************************
sub MergeDuplicateObjectListByName
{
   my $sourceListRef = shift;
   my $resultListRef = shift;
   my $mergedListRef = shift;

   my %objectHash;

   foreach my $object (@$sourceListRef)
   {
      if (exists $objectHash{$object->{"name"}})
      {
         $objectHash{$object->{"name"}}{"size"} += $object->{"size"};
         push @$mergedListRef, $objectHash{$object->{"name"}};
      }
      else
      {
         $objectHash{$object->{"name"}} = $object;
      }
   }

   @$resultListRef = values %objectHash;
}
...
#****************************************************************************
# subroutine:  GetROCodeWildcard
# description: Get the object of RO-CODE wildcard.
# input:       None
# output:      RO-CODE wildcard object
# need init:   No
#****************************************************************************
sub GetROCodeWildcard
{
   return &objListHelper::RO_CODE_WILDCARD;
}

#****************************************************************************
# subroutine:  GetRODataWildcard
# description: Get the object of RO-DATA wildcard.
# input:       None
# output:      RO-CODE wildcard object
# need init:   No
#****************************************************************************
sub GetRODataWildcard
{
   return &objListHelper::RO_DATA_WILDCARD;
}
...
# description: Get the whole file content.
# input:       Filename
# output:      File content
# need init:   No
#****************************************************************************
...
{
   my $filename = shift;
   return &CommonUtil::GetFileContent($filename);
}
...
sub BackupFile
{
   ...
   my $backupFilename = $filename.".".$keyword.&GetTimeStamp();

   copy($filename, $backupFilename) or
      &ErrorHandler("Cannot backup file $filename: $!");
