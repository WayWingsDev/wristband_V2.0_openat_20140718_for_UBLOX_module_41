...
my $OBJ_LIST_HELPER_VERNO = " v1.03";
   # v1.04: Support RO-CODE/RO-DATA wildcard
...
# Constants
#****************************************************************************
use constant RO_CODE_WILDCARD => {"name" => "*", "attributes"=> ["+RO-CODE"], "category" => "Wildcard"};
use constant RO_DATA_WILDCARD => {"name" => "*", "attributes"=> ["+RO-DATA"], "category" => "Wildcard"};
...
sub InputObjectListFromCSVFile
{
   ...
   &ErrorHandler("Undefined object list reference!", __FILE__, __LINE__) unless (defined $objectListRef);
    ...
    my ($ContentList_aref, $Index_href) = &CommonUtil::ParseCSV($filename, undef);
    foreach my $item (@$ContentList_aref)
    {
        my $strModuleName = $item->[$Index_href->{ModuleName}];
        my $strAttribute = $item->[$Index_href->{Attribute}];
        my $strFeatureOption = $item->[$Index_href->{FeatureOption}];
        my $strCategory = (exists $Index_href->{Category}) ? $item->[$Index_href->{Category}] : "";
        $strModuleName =~ s/\s//g;
        next if($strModuleName eq "" || $strModuleName eq "*");
        if($strModuleName =~ /.lib$|.a$/ and $strModuleName !~ /^\*|\;/)
        {
            $strModuleName = "*".$strModuleName;
        }
        # Check the feature option
        if (($strFeatureOption ne "") and defined $makefileOptionsHashRef)
        {
            next unless (&FileInfo::EvaluateFeatureOptionCondition($strFeatureOption, $makefileOptionsHashRef));
        }

        my @AttrTemp = split(/\s+/, $strAttribute);
        my @attributeArray = map{"+$_" }@AttrTemp;

        my %object = ("name" => $strModuleName,
                      "attributes" => \@attributeArray,
                      "library" => $item->[$Index_href->{LibraryName}],
                      "category" => $strCategory,
                      "feature_option" => $strFeatureOption);
        push @$objectListRef, \%object;
    }
}
...
sub OutputObjectListToCSVFile
{
   ...
   print $output "ModuleName,LibraryName,Attribute,Category,FeatureOption\n";

   foreach my $objectRef (@$objectListRef)
   {
      print $output "$objectRef->{name},$objectRef->{library},";
      my @attribute = map{$_=~s/\+//; $_; }@{$objectRef->{"attributes"}};
      print $output join(' ', @attribute);
      print $output ",$objectRef->{category},$objectRef->{feature_option}\n";
   }
...
sub MergeAttributeList
{
   ...
   @$firstRef = sort compareAttributeSort keys %resultHash;
}

sub compareAttributeSort
{
   my %attributeToNumber = ("+RO" => 0, "+RO-CODE" => 1, "+RO-DATA" => 2);

   return $attributeToNumber{$a} <=> $attributeToNumber{$b};
}
...
sub IsObjectAttributeListConflict
{
   my $firstObject = shift;
   my $secondObject = shift;

   return &isAttributeListConflict($firstObject->{"attributes"}, $secondObject->{"attributes"});
}

sub isAttributeListConflict
{
   my $firstList = shift;
   my $secondList = shift;

   my %firstHash = map { $_ => 1 } @$firstList;
   my %secondHash = map { $_ => 1 } @$secondList;

   if (defined $firstHash{"+RO"} and defined $secondHash{"+RO"})
   {
      return 1;
   }

   my @attributeType = ("+RO-CODE", "+RO-DATA");

   foreach (@attributeType)
   {
      return 1 if (defined $firstHash{$_} and defined $secondHash{$_});
   }

   return 0;
}

sub RemoveDuplicateObjectAttribute
{
   my $firstObject = shift;
   my $secondObject = shift;

   &removeDuplicateAttribute($firstObject->{"attributes"}, $secondObject->{"attributes"});
}

sub removeDuplicateAttribute
{
   my $firstList = shift;
   my $secondList = shift;

   my %firstHash = &changeAttributeListToHash($firstList);
   my %secondHash = &changeAttributeListToHash($secondList);

   @$firstList = grep((not defined $secondHash{$_}), &changeAttributeHashToList(\%firstHash));
}

sub changeAttributeListToHash
{
   my $attributeList = shift;
   my %attributeHash = map { $_ => 1 } @$attributeList;

   if ($attributeHash{"+RO"})
   {
      delete $attributeHash{"+RO"};
      $attributeHash{"+RO-CODE"} = 1;
      $attributeHash{"+RO-DATA"} = 1;
   }

   return %attributeHash;
}

sub changeAttributeHashToList
{
   my $attributeHash = shift;
   return sort compareAttributeSort keys %$attributeHash;
}
