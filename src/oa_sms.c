...
//oa_sms_for_wristband g_current_sms_for_wristband = OA_SMS_WRISTBAND_NULL;
oa_uint8 g_current_band_number = 0;
...
void oa_sms_list_parse(const oa_char *buf, oa_sms_list_entry *entry)
{
    ...
    entry->index = atoi(unsol_p.param[0]);
    entry->state = atoi(unsol_p.param[1]);
    entry->length = atoi(unsol_p.param[3]);
...
{
    oa_sms_send("15365007182", "123ABC", oa_strlen((oa_char *)"123ABC"),OA_SMSAL_DEFAULT_DCS);
...
void os_sms_send_demo_8bit(void)
{
    ...
    oa_sms_send("15365007182",  (oa_char *)smsSendData, 6,OA_SMSAL_8BIT_DCS);
...
void os_sms_send_demo_ucs2(void)
{
    ...
    oa_sms_send("15365007182", sendUcs2Str, oa_wstrlen((oa_wchar *)sendUcs2Str),OA_SMSAL_UCS2_DCS);
...
oa_bool oa_band_number_check(oa_char *deliver_num)
{
    oa_uint8 i = 0;
    oa_uint8 deliver_num_len = oa_strlen(deliver_num);
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        {
            OA_DEBUG_USER("%s->OA_TRUE", __func__);
            g_current_band_number = i;
            return OA_TRUE;
        }
    }
    OA_DEBUG_USER("%s->OA_FALSE", __func__);
    return OA_FALSE;
}
...
oa_bool oa_sms_rcv_ind_handler(oa_char * deliver_num, oa_char * timestamp, oa_uint8 * data, oa_uint16 len, oa_smsal_dcs_enum dcs)
{
    ...
    if (OA_TRUE == oa_band_number_check(deliver_num))
    {
        if(dcs == OA_SMSAL_DEFAULT_DCS)
    	{
            ...
            //!!!notice: must be this format:             #IP:221.226.102.18:28081  || #FTP:221.226.102.18
            if ((0 == oa_strncmp(data, "#IP", 3))||(0 == oa_strncmp(data, "#ip", 3))
              ||(0 == oa_strncmp(data, "#Ip", 3))||(0 == oa_strncmp(data, "#iP", 3)))
            {
                //ip&port
                pHead = oa_strchr(data, ':');
                if (pHead == NULL)
                {
                    return OA_TRUE;
                }
                ...
                oa_memcpy(buf, pHead, pTail - pHead);
                if (OA_FALSE == oa_IP_isValid(buf))
                {
                    return OA_TRUE;
                }
                else
                {
                    oa_soc_close_req();
                    oa_memset(g_soc_param.serve_ipaddr, 0, sizeof(g_soc_param.serve_ipaddr));
                    oa_memcpy(g_soc_param.serve_ipaddr, pHead, pTail - pHead);
                    pHead = ++pTail;
                    g_soc_param.port = oa_atoi(pHead);
                    OA_DEBUG_USER("config ip:%s,port:%d", g_soc_param.serve_ipaddr, g_soc_param.port);
                    oa_soc_setting_save(SETTING_SOC);
                    //oa_module_restart(NULL);
                }
            }
            else if ((0 == oa_strncmp(data, "#FTP", 4))||(0 == oa_strncmp(data, "#ftp", 4)))
            {
                pHead = oa_strchr(data, ':');
                if (pHead == NULL)
                {
                ...
                {
                    oa_memset(g_oa_wristband_parameter.ftp, 0, sizeof(g_oa_wristband_parameter.ftp));
                    oa_memcpy(g_oa_wristband_parameter.ftp, buf, oa_strlen(buf));
                    OA_DEBUG_USER("ftp:%s->buf:%s", g_oa_wristband_parameter.ftp, buf);
    	...
    	else if(dcs == OA_SMSAL_UCS2_DCS)
    	{
            ...
            if (0 == oa_strcmp((oa_char *)rawData, "#状态"))
            {
                OA_DEBUG_USER("#状态");
                oa_sms_handler_for_wristband(OA_SMS_WRISTBAND_STATUS);
            }
            else if (0 == oa_strcmp((oa_char *)rawData, "#定位"))
            {
                OA_DEBUG_USER("#定位");
                oa_sms_handler_for_wristband(OA_SMS_WRISTBAND_LOCATE);
            }
            else if (0 == oa_strcmp((oa_char *)rawData, "#监听"))
            {
                OA_DEBUG_USER("#监听");
                oa_sms_handler_for_wristband(OA_SMS_WRISTBAND_MONITOR);
            }
            else if (0 == oa_strcmp((oa_char *)rawData, "#升级"))
            {
                ...
                {
                    OA_DEBUG_USER("#升级->低电量无法升级");
                }
            }
            else if (0 == oa_strcmp((oa_char *)rawData, "#重启"))
            {
                OA_DEBUG_USER("#重启");
                oa_module_restart(NULL);
            }
            else if (0 == oa_strcmp((oa_char *)rawData, "#打开报警"))
            {
                OA_DEBUG_USER("#打开报警");
                g_oa_wristband_parameter.alarm = 1;
                oa_soc_setting_save(SETTING_WRISTBAND);
            }
            else if (0 == oa_strcmp((oa_char *)rawData, "#关闭报警"))
            {
                OA_DEBUG_USER("#关闭报警");
...
void oa_sms_handler_for_wristband(oa_sms_for_wristband mode)
{
    ...
    oa_char gpsstatus[2][10] = {"关闭","开启"};
    oa_char alarmstatus[2][10] = {"关闭","开启"};
    oa_uint8 buf[14] = {0};
    oa_uint16 crcvalue = 0;
    
    if (oa_at_busy()||oa_is_in_call())
    {
        return;
    }
    
    if (mode == OA_SMS_WRISTBAND_NULL)
    {
        return;
    }
    else
    {
        switch (mode)
        {
            case OA_SMS_WRISTBAND_STATUS:
                OA_DEBUG_USER("OA_SMS_WRISTBAND_STATUS");
                oa_sprintf(sendStr, "模式:%s，频率:%d，电量:%d，充电:%s，拆解:%s，报警:%s，网络:%s，GPS:%s，Ver:%x，SIM:%d，CREG:%d",
                                    workmode[g_oa_wristband_parameter.mode],
                                    g_oa_wristband_parameter.period,
                                    g_BatteryPercent,
                                    chargingstatus[g_ChargeStatus],
                                    disassemblystatus[g_DisassemblyStatus],
                                    alarmstatus[g_oa_wristband_parameter.alarm],
                                    loginstatus[g_LoginStatus],
                                    gpsstatus[g_gps_poweron],
                                    g_versionL,
                                    oa_sim_network_is_valid(),
                                    oa_network_get_signal_level(NULL));
            ...
            case OA_SMS_WRISTBAND_LOCATE:
                OA_DEBUG_USER("OA_SMS_WRISTBAND_LOCATE");
                if (2 == g_LoginStatus)
                {
                    buf[0] = 0xFF;
                    buf[1] = 0xFF;
                    buf[2] = 0x11;
                    buf[3] = g_serial_number/256;
                    buf[4] = g_serial_number%256;
                    ...
                    buf[5] = 0x06;
                    buf[6] = g_current_band_number + 1;//reserve
                    buf[7] = 0x00;
                    buf[8] = 0x00;
                    crcvalue = u16CRC_Calc16(buf, 9);
                    buf[9] = crcvalue/256;
                    buf[10] = crcvalue%256;
                    buf[11] = 0xEE;
                    buf[12] = 0xEE;
                    oa_soc_send(g_soc_context.socket_id, buf, 13);
                }
                else
                {
                    OA_DEBUG_USER("has not login");
                }
                break;

            case OA_SMS_WRISTBAND_MONITOR:
                OA_DEBUG_USER("OA_SMS_WRISTBAND_MONITOR");
                oa_sprintf(sendStr, "ATD%s;\r\n", g_oa_wristband_parameter.phonenumber[g_current_band_number]);
                oa_at_cmd_demo(sendStr);
                oa_at_cmd_demo_submit();
                break;
