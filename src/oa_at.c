...
{
    oa_memset(llp, 0x00, sizeof(llparser));
    llp->state = LLPARSE_STATE_IDLE;
    llp->cur = llp->buf;
...
void oa_at_response_parse(void *param)
{
    ...
    if (buf[0] == '+') {
        ...
        /*bluetooth:*/
        if(!oa_strncmp(buf+1, "BT:", 3)){
            //ata OR ath
            //OA_DEBUG_USER("BT entry here.");
            if(!oa_strncmp(buf+5, "PASSCODE CONFIRM YES OR NO?", oa_strlen("PASSCODE CONFIRM YES OR NO?")))
            {
                oa_at_cmd_demo("AT+BTCTRL=2,1\r\n");
                oa_at_cmd_demo_submit();
            }
            if(!oa_strncmp(buf+5, "ACCEPT SERIAL PORT YES OR NO?", oa_strlen("ACCEPT SERIAL PORT YES OR NO?")))
            {
                oa_sleep(200);
                oa_at_cmd_demo("AT+BTCTRL=3,1\r\n");
                oa_at_cmd_demo_submit();
            }
...
void oa_clear_lbs(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        g_oa_lbs_parameter_next[i].mcc = 0;
        g_oa_lbs_parameter_next[i].mnc = 0;
        g_oa_lbs_parameter_next[i].lac = 0;
        g_oa_lbs_parameter_next[i].ci = 0;
        g_oa_lbs_parameter_next[i].rssi = 0;
    }
}

void oa_save_lbs(void)
{
    oa_uint8 i = 0;

    for (i = 0; i < 6; i++)
    {
        g_oa_lbs_parameter_pre[i].mcc = g_oa_lbs_parameter_next[i].mcc;
        g_oa_lbs_parameter_pre[i].mnc = g_oa_lbs_parameter_next[i].mnc;
        g_oa_lbs_parameter_pre[i].lac = g_oa_lbs_parameter_next[i].lac;
        g_oa_lbs_parameter_pre[i].ci = g_oa_lbs_parameter_next[i].ci;
        g_oa_lbs_parameter_pre[i].rssi = g_oa_lbs_parameter_next[i].rssi;
    }
}

oa_bool oa_check_lbs_change(void)
{
    oa_uint8 i = 0;

    for (i = 0; i < 6; i++)
    {
        if ((g_oa_lbs_parameter_pre[i].lac != g_oa_lbs_parameter_next[i].lac)
          ||(g_oa_lbs_parameter_pre[i].ci != g_oa_lbs_parameter_next[i].ci))
          //||(g_oa_lbs_parameter_pre[i].rssi != g_oa_lbs_parameter_next[i].rssi))
        {
            return OA_FALSE;
...
void oa_app_at_rsp_recv(oa_uint16 len, oa_uint8 *pStr)
{
    ...
    LBS_Debug("%s called: %s,%d", __func__, pStr, len);

	/* TODO : add support for user port.*/
    if (0 == oa_strncmp(pStr, "\r\n+CCED:", 8))
    {
        if (len < 20)
        {
        }
        else
        {
            ...
            oa_memset(temp, 0x00, sizeof(temp));
            oa_memcpy(temp, pHead, pTail - pHead);
            g_oa_lbs_parameter_next[g_oa_at_cced_NO].mcc = oa_atoi(temp);
            LBS_Debug("mcc=%d", g_oa_lbs_parameter_next[g_oa_at_cced_NO].mcc);
            ...
            oa_memset(temp, 0x00, sizeof(temp));
            oa_memcpy(temp, pHead, pTail - pHead);
            g_oa_lbs_parameter_next[g_oa_at_cced_NO].mnc = oa_atoi(temp);
            LBS_Debug("mnc=%d", g_oa_lbs_parameter_next[g_oa_at_cced_NO].mnc);
            ...
            oa_memset(temp, 0x00, sizeof(temp));
            oa_memcpy(temp, pHead, pTail - pHead);
            g_oa_lbs_parameter_next[g_oa_at_cced_NO].lac = oa_strtoul(temp, NULL, 16);
            LBS_Debug("lac=%d", g_oa_lbs_parameter_next[g_oa_at_cced_NO].lac);
            ...
            oa_memset(temp, 0x00, sizeof(temp));
            oa_memcpy(temp, pHead, pTail - pHead);
            g_oa_lbs_parameter_next[g_oa_at_cced_NO].ci = oa_strtoul(temp, NULL, 16);
            LBS_Debug("cellid=%x", g_oa_lbs_parameter_next[g_oa_at_cced_NO].ci);
            ...
            pHead = pTail + 1;
            oa_memset(temp, 0x00, sizeof(temp));
            oa_strcpy(temp, pHead);
            g_oa_lbs_parameter_next[g_oa_at_cced_NO].rssi = oa_atoi(temp);
            LBS_Debug("rssi=%d", g_oa_lbs_parameter_next[g_oa_at_cced_NO].rssi);

            g_oa_at_cced_NO++;
        }
    }
    else if((0 == oa_strncmp(pStr, "\r\nOK", 4))||(0 == oa_strncmp(pStr, "\r\nok", 4)))
    {
        LBS_Debug("g_oa_at_cced_NO=%d", g_oa_at_cced_NO);
    }
...
void oa_at_init(void)
{
    ...
    oa_memset(&g_at_cmd_list, 0x00, sizeof(llist_head));
    oa_memset(&g_dummy_at_cmd, 0x00, sizeof(oa_at_cmd));
